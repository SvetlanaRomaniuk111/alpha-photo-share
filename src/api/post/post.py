...
from src.schemas.post import PostCreationSchema, PostUpdateSchema, PostResponse
...
async def create_post(body: PostCreationSchema, db: AsyncSession = Depends(get_db),
                         user: User = Depends(all_roles_access)):


    post = await repositories_posts.create_post(body, user, db)
    return post
...
async def update_post(body: PostUpdateSchema, post_id: UUID,user: User = Depends(all_roles_access), db: AsyncSession = Depends(get_db)):
    ...
    post = await repositories_posts.update_post(body, post_id, user, db)
    if post is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Post not found")
    return post
...
async def delete_post( post_id: UUID, user: User = Depends(all_roles_access), db: AsyncSession = Depends(get_db)):
    ...
    post = await repositories_posts.delete_post(post_id, user, db)
    if post is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="post not found")
